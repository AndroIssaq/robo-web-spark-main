...
const ServicesSection = ({ data }: ServicesSectionProps) => {
  return (
    ...
    >
      ...
      >
        {data.services.list.map((ser: any, i: number) => (
          <motion.div
            key={i}
            variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }}
            whileHover={{ y: -6, transition: { duration: 0.3 } }}
            className="flex flex-col md:flex-row items-center md:items-stretch bg-white dark:bg-neutral-950 rounded-2xl border border-mint/10 shadow-md overflow-hidden"
          >
            
            {/* Image Section */}
            <div className={`flex-shrink-0 flex justify-center items-center md:w-2/5 p-6 ${i % 2 !== 0 ? 'md:order-last' : ''}`}>
              <img
                src={ser.img} 
                alt={ser.title}
